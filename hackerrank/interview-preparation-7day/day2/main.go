...
func main() {
	testCase_flipping_the_matrix()
}

//day 2

//mock testCase
func testCase_flipping_the_matrix() {
	matrix := [][]int32{
		{1, 2},
		{3, 4},
	}

	fmt.Println(flipping_the_matrix(matrix))
	...
	//[i][j], [i][n-1-j], [n-1-i][j], [n-1-i][n-1-j], i,j,n = 0,0,4
	//  [0][0], [0][3], [3][0], [3][3]
	// step 1 107, 15, 75, 112 -> 112

	//[i][j], [i][n-1-j], [n-1-i][j], [n-1-i][n-1-j],  i,j,n = 0,1,4
	// [0][1], [0][2], [3][1], [3][2]
	// step 2 54, 128, 15, 28 -> 128

	//[i][j], [i][n-1-j], [n-1-i][j], [n-1-i][n-1-j],  i,j,n = 1,0,4
	// [1][0], [1][3], [2][0], [2][3]
	// step 3 12, 138, 100, 85 -> 138

	//[i][j], [i][n-1-j], [n-1-i][j], [n-1-i][n-1-j],  i,j,n = 1,1,4
	// [1][1], [1][2], [2][1], [2][2]
	// step 4 75, 110, 96, 34 -> 110
	// step 5 -> 488

	fmt.Println(flipping_the_matrix(matrix)) //expected 488
}

func flipping_the_matrix(matrix [][]int32) int32 {
	var res int32
	n := len(matrix)

	for i := 0; i < n/2; i++ { //matrix is 2n x 2n
		// find max in each edge
		for j := 0; j < n/2; j++ {
...
func countingSort_1(arr []int32) []int32 {
	res_arr := make(map[int32]int32)

	res := make([]int32, len(arr)) //0..len(arr)

	for _, v := range arr { //assign hash map
		res_arr[v]++
	}
	...
	// nres := len(res)-1      // didn't show another zero after last element
	// for ; nres != 0; nres--{
	// 	if res[nres] > 0{
	// 		break
	// 	}
	...
	return res[:100] //make sure to read problem
}

func testCase_diagonalDifference() {
	arr := [][]int32{
		{1, 2, 3},
		{4, 5, 6},
		{9, 8, 9},
	}
	fmt.Println(diagonalDifference(arr)) // expected 1+5+9 15 - 3+5+9 17 -> 2
}
